...
public class MovingRotorTest {
    ...
    public void testConvertForward() {
        ...
        Permutation perm = new Permutation("(AELTPHQXRU) (BKNW)"
                + "(CMOY) (DFG) (IV) (JZ) (S)", az);
        Rotor one = new MovingRotor("test", perm, "Q");
        assertEquals(one.convertForward('A'), 'W'
                - perm.alphabet().toChar(0));
        assertEquals(one.convertForward('I'), 'I'
                - perm.alphabet().toChar(0));
        assertEquals(one.convertForward('O'), 'K'
                - perm.alphabet().toChar(0));
    }
    ...
    public void testConvertBackward() {
        Alphabet az = new CharacterRange('A', 'Z');
        Permutation perm = new Permutation("(FIXVYOMW) (CDKLHUP) "
                + "(ESZ) (BJ) (GR) (NT) (A) (Q)", az);
        Rotor one = new MovingRotor("test", perm, "E");
        assertEquals(one.convertBackward('X'), 'D'
                - perm.alphabet().toChar(0));
        assertEquals(one.convertBackward('C'), 'U'
                - perm.alphabet().toChar(0));
        assertEquals(one.convertBackward('V'), 'F'
                - perm.alphabet().toChar(0));
    }
